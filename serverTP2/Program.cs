...
class Servidor
{
    ...
    {
        string servicesFilePath = Path.Combine(Directory.GetCurrentDirectory());
        try
        {
        ...
        {
            Console.WriteLine($"Erro ao carregar dados do arquivo CSV {servicesFilePath}: {ex.Message}");
        }
    }
    ...
    public static string AllocateTask(string clientId)
    {
        ...
        try
        {
            ...
            if (taskDict.ContainsKey(serviceId) && taskDict[serviceId].Count > 0)
            {
                string taskDescription = taskDict[serviceId][0];
                taskDict[serviceId].RemoveAt(0);
                string message = $"TASK_ALLOCATED:{taskDescription}";
    ...
    public static string MarkTaskAsCompleted(string clientId, string taskDescription)
    {
        ...
        try
        {
            ...
            string serviceId = serviceDict[clientId];
            if (taskDict.ContainsKey(serviceId) && taskDict[serviceId].Contains(taskDescription))
            {
                taskDict[serviceId].Remove(taskDescription);
